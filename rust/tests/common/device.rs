...
use libosdp::{ControlPanel, events::OsdpEvent, PdInfo, OsdpFlag, channel::{OsdpChannel, Channel}, PdId, PdCapability, PdCapEntity, PeripheralDevice, commands::OsdpCommand};
...
impl CpDevice {
    pub fn new<T: Channel + 'static>(bus: T) -> Result<Self> {
        ...
        let _ = thread::Builder::new().name("CP Thread".to_string()).spawn(move || {
            let dev = dev_clone;
            let sender = event_tx;
...
impl PdDevice {
    pub fn new<T: Channel + 'static>(bus: T) -> Result<Self> {
        ...
        let dev_clone = dev.clone();
        let _ = thread::Builder::new().name("PD Thread".to_string()).spawn(move || {
            let dev = dev_clone;
            let sender = cmd_tx;
