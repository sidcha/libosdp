...
use libosdp::{
    commands::{OsdpCommand, OsdpCommandBuzzer},
    events::{OsdpEvent, OsdpEventCardRead},
    ControlPanel, PeripheralDevice, channel::{Write, Read},
};

use crate::common::{threadbus::ThreadBus, MemoryChannel, device::CpDevice, device::PdDevice};
...
fn test_commands() -> Result<()> {
    common::setup();
    // let pd_bus = ThreadBus::new("conn-0");
    // let cp_bus = pd_bus.clone();
